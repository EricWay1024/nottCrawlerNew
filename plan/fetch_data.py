...
def get_all_plans(campus, year):
    ...
    for row in rows:
        ...
        plan_code = row.find(
            "span",
            id=lambda x: x and x.startswith("UN_PAM_PLN_TBL_DESCRSHORT"),
        ).get_text(strip=True)

        # Extract the Link Code (e.g., U6UMTHPY, U7UMTHPY)
        link_code = row.find(
            "a", id=lambda x: x and x.startswith("HYPERLINK")
        ).get_text(strip=True)

        # Extract the Plan Description (e.g., BSc Hons Mathematical Physics)
        plan_desc = row.find(
            "span",
            id=lambda x: x and x.startswith("UN_PAM_PLN_TBL_TRNSCR_DESCR"),
        ).get_text(strip=True)

        # Extract additional Notes (e.g., U6UMTHPY (UCAS:F326))
        notes = row.find(
            "span",
            id=lambda x: x and x.startswith("UN_PLN_EXT2_WRK_DESCRLONG_NOTES"),
        ).get_text(strip=True)
...
def parse_modules(soup):
    ...
    def process_group(group_soup, type):
        ...
        if type == "Compulsory":
            ...
            return {
                ...
                "message": ge_(
                    r"^win0divUN_PAM_PLAN_WRK_UN_RESTRICT_MSG\$\d+$"
                ),
                **res,
            }
            ...
            raise ValueError(
                "Invalid group type. Must be Compulsory, Restricted or Alternative."
            )
    ...
    def is_empty_group(group_obj):
        return (
            group_obj["message"] == ""
            and group_obj["title"] == ""
            and group_obj["modules"] == []
        )

    def get_all_groups_from_year(year_soup):
        comp_groups = year_soup.find_all(
            id=rc(r"^win0divUN_PAM_PLAN_WRK_UN_PAM_COMPULSORY\$\d+$")
        )
        rest_groups = year_soup.find_all(
            id=rc(r"^win0divUN_PAM_RES_TBL_row\$\d+$")
        )
        altr_groups = year_soup.find_all(
            id=rc(r"^win0divUN_PAM_ALTR_TBL_row\$\d+$")
        )
        return list(
            filter(
                lambda g: not is_empty_group(g),
    ...
    for year_index, year_soup in enumerate(years):
        year_obj = {
            ...
            "additionalCourseChoice": ge(
                year_soup,
                rc(r"^win0divUN_PAM_ADDT_TBL_SSR_INSTRCTN_LONG\$\d+$"),
            ),
        }
        res.append(year_obj)
    return res
...
def get_plan(plan_code, year, campus):
    response = requests.get(
        f"https://campus.nottingham.ac.uk/psc/csprd_pub/EMPLOYEE/HRMS/c/UN_PROG_AND_MOD_EXTRACT.UN_PLN_EXTRT_FL_CP.GBL?PAGE=UN_PLN_EXT3_FPG&CAMPUS={campus}&TYPE=Programme&YEAR={year}&TITLE=UON-e&PLAN={plan_code}&UCAS=",
        timeout=10,
    )
    soup = BeautifulSoup(response.text, "html.parser")
    ...
    try:
        ...
        print(
            f"JSON schema validation error for plan {plan_code} of campus {campus}, {year}:"
        )
        print(e)
    return plan_res
